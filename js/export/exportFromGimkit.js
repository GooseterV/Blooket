...
const gimkitID = document.URL.split("view/")[1];
async function getGimkitSet(id) {
	const set = await fetch(`https://www.gimkit.com/api/games/fetch/${id}`);
...
async function exportGimkitSet(id) {
	const set = (await getGimkitSet(id));
