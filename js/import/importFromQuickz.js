...
const setData = JSON.parse(b64decode(iF.contentWindow.prompt("What is your quickz deck data? (Exported from exportFromQuickz.js)")));
...
async function importFromQuickz(setData) {
	const blooketSetPayload = {
		"title":setData.name,
		"author":localStorage.washere,
		"desc":setData.description.slice(0, 300),
		"coverImage":{
			"url":setData.image.url
		},
		"private":false
	};
	const newSet = await createSet(blooketSetPayload);
	const quickzQuestions = setData.questions;
	for (let question of quickzQuestions) {
		const answers = [].concat(...question.answers).map(({text})=>text);
		const correctAnswers = [].concat([].concat(...question.answers).filter(choice => choice.correct)).map(({text})=>text);
